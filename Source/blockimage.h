...
public:
    BlockImage(const ConfigData &data, QWidget *parent = nullptr) :
        QWidget(parent),
        mMediaLoad(new MediaLoad(MediaLoad::Image, this)),mSetting(new Setting(data, this))
    {
        ...
        mSetting->hideAll();
        mSetting->toggleMaxSizeShow();
        mSetting->toggleExportPathShow();
        mSetting->toggleFilterMethodShow();
        ...
        connect(mGenerateBtn, &QPushButton::clicked, this, &BlockImage::generate);
    }
    ~BlockImage() {};
...
private:
    void generate() {
        ...
        std::cerr << "Input Path: " << inputPath << std::endl;
        std::cerr << "Output Path: " << outputPath << std::endl;
        std::cerr << "Raws Size: " << raws.size() << std::endl;
        std::cerr << "Modis Size: " << modis.size() << std::endl;
        std::cerr << "Plane: " << data.plane << std::endl;
        std::cerr << "Attribute: " << data.filterAttribute << std::endl;
        makeBlockImage(inputPath, outputPath, modis, "./resource/textures/bedrock",
                       data.maxSize[0], data.maxSize[1]);
    }
