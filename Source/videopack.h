...
class VideoPack : public QWidget
{
    Q_OBJECT

public:
    VideoPack(const ConfigData &data, QWidget *parent = nullptr) :
        QWidget(parent),
        mMediaLoad(new MediaLoad(MediaLoad::Video, this)), mSetting(new Setting(data, this))
    {
        ...
        mMediaLoad->setVideoThumb(QPixmap(":/image/resource/image/video.png"));
        ...
        mSetting->toggleFunctionPackShow();
        mSetting->toggleExportMethodShow();
        ...
        connect(mGenerateBtn, &QPushButton::clicked, this, &VideoPack::generate);
    }
    ~VideoPack() {};
...
private:
    void generate() {
        ...
        makeVideoStructurePack(inputPath, outputPath, modis, data.manifest, data.plane,
                               data.maxSize[0], data.maxSize[1], data.maxFrameCount, data.detachFrame,
                               data.compress);
    }
